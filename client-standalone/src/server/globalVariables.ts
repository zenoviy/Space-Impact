function initAppGlobalVariable(){
    ...
    if(process.env.NODE_ENV === 'development') process.env.HOST = __dirname + '/';
    else if(process.env.NODE_ENV === 'production'){ process.env.HOST = __dirname + '/'; console.log("production mode check HOST")};
}
